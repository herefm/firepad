...
firepad.RichTextToolbar = (function(global) {
  ...
  // TODO Can get rid of imageInsertionUI
  ...
  utils.makeEventEmitter(RichTextToolbar, ['bold', 'italic', 'underline', 'strike', 'font', 'font-size', 'color',
    'left', 'center', 'right', 'unordered-list', 'ordered-list', 'indent-increase', 'indent-decrease']);
  ...
  RichTextToolbar.prototype.show = function(cursorCoords) {
    this.element_.style.display = 'block';
    this.element_.style['margin-left'] = (cursorCoords.left - 100) + 'px';
    this.element_.style['margin-top'] = (cursorCoords.top - 70) + 'px';
  }
  ...
  RichTextToolbar.prototype.makeFontSizeDropdown_ = function() {
    ...
    for(var i = 0; i < sizes.length; i++) {
      var content = utils.elt('span', sizes[i][1]);
      content.setAttribute('style', 'line-height: 2.1em;padding: 5px;font-size:' + sizes[i][0]);
      items.push({ content: content, value: sizes[i][0] });
    }
  ...
  RichTextToolbar.prototype.makeColorDropdown_ = function() {
    ...
    //var colors = ['black', 'grey', 'red', 'green', 'blue', 'yellow', 'cyan', 'magenta'];
...
firepad.EditorClient = (function () {
  ...
  EditorClient.prototype.onCursorActivity = function () {
    ...
    if (this.cursor.position != this.cursor.selectionEnd) {
      if (this.onTextSelected) {
        let scrollInfo = this.editorAdapter.cm.getScrollInfo();
        var cursorPos = this.editorAdapter.cm.posFromIndex(this.cursor.position);
        var cursorCoords = this.editorAdapter.cm.cursorCoords(cursorPos, 'local');

        // Adjust cursor coordinates to screen coordinates
        cursorCoords.top -= scrollInfo.top;

        this.onTextSelected(cursorCoords);
      }
    } else {
...
firepad.Firepad = (function(global) {
  ...
  Firepad.prototype.dispose = function() {
    ...
    if (this.firepadWrapper_.parentNode) {
